...
try: visual_service = VisualService()
...
except Exception as e: SDK_READY = False
SDK_MODULE = f "failed: {str(e)}"
def authenticate_request(): auth_header = request.headers.get('Authorization')
if not auth_header or not auth_header.startswith('Bearer '): return False, "Missing or invalid Authorization header"
token = auth_header[7: ]
if token != PROXY_AUTH_TOKEN: return False, "Invalid authentication token"
...
def generate_video(): auth_success, auth_message = authenticate_request()
if not auth_success: return jsonify(
{
    "success": False,
    "error": auth_message
}), 401
...
if not SDK_READY: return jsonify(
{
    "success": True,
    "status": "simulated",
    "sdk_used": "simulated",
...
if response.get('code') == 10000: task_id = response.get('data',
{}).get('task_id', '')
return jsonify(
{
    "success": True,
...
if token != PROXY_AUTH_TOKEN: return jsonify(
{
    "success": False,
...
except Exception as e: print(f "[ERROR] 查询任务状态失败: {str(e)}")
task_id = "unknown"#
修复： 确保task_id变量存在
return jsonify(
{
